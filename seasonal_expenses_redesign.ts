// Nouvelle structure pour les dépenses saisonnières
interface SeasonalExpense {
  id: string;
  category: 'automobile' | 'maison' | 'sante' | 'taxes' | 'personnel';
  name: string;
  description: string;
  isActive: boolean; // Case à cocher "S'applique à moi"
  estimatedAmount: number;
  frequency: 'annually' | 'biannually' | 'every2years' | 'every3years' | 'every5years' | 'asNeeded';
  lastOccurrence?: string; // Date de la dernière fois
  nextDue?: string; // Date prévue de la prochaine occurrence
  isPlanned: boolean; // Si planifié pour l'année en cours
  plannedDate?: string; // Date planifiée cette année
  notes?: string;
}

// Données prédéfinies avec logique d'applicabilité
...
// Composant pour la gestion des dépenses saisonnières
const SeasonalExpensesManager: React.FC = () => {
  const [expenses, setExpenses] = useState<SeasonalExpense[]>(seasonalExpensesData);
  ...
  const getAnnualBudgetImpact = () => {
    return expenses
      .filter(exp => exp.isActive)
      .reduce((total, exp) => {
        if (exp.isPlanned) {
          return total + exp.estimatedAmount;
        }
        // Pour les dépenses récurrentes, calculer l'impact annuel moyen
        const frequencyMultiplier = {
          ...
          'every3years': 0.33,
          'every5years': 0.2,
          'asNeeded': 0 // Ne pas inclure dans le calcul automatique
        };
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <span className="text-sm text-gray-500">
                    {expense.frequency === 'annually' && 'Annuel'}
                    {expense.frequency === 'biannually' && 'Deux fois/an'}
                    {expense.frequency === 'every2years' && 'Aux 2 ans'}
                    {expense.frequency === 'every3years' && 'Aux 3 ans'}
                    {expense.frequency === 'every5years' && 'Aux 5 ans'}
                    {expense.frequency === 'asNeeded' && 'Selon besoin'}
                  </span>
                </div>
              </div>
            </div>
